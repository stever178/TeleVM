...
impl TestSock {
    pub fn new(path: &str, controller: Arc<Mutex<dyn MachineTestInterface>>) -> Self {
        let stream = match UnixStream::connect(path) {
            Ok(s) => s,
            Err(e) => {
                println!("Failed to connect test socket: {}", e); // 打印完整的错误信息
...
fn handle_test_cmd(stream_fd: RawFd, controller: &Arc<Mutex<dyn MachineTestInterface>>) {
    ...
    match cmd[0] {
        "read" => {
            ...
            // println!("~ in handler read, addr is 0x{}, size is 0x{}, data is {:?}", &cmd[1].trim_start_matches("0x"), &cmd[2].trim_start_matches("0x"), &data);
            handler
        ...
        "write" => {
            ...
            // println!("~ in handler write, addr is 0x{}, size is 0x{}, data_str is {}", &cmd[1].trim_start_matches("0x"), &cmd[2].trim_start_matches("0x"), data_str);
